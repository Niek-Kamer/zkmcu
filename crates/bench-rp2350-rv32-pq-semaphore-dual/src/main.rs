...
// 384 KB heap. Same budget as the M33 sibling — the dual entry point
// drops each leg's parsed proof before the next leg starts, so peak
// heap is `max(p2_peak, b3_peak)` ≈ 330 KB. Leaves ~120 KB stack
// region for `measure_stack_peak`'s 64 KB sentinel paint.
...
pub static PICOTOOL_ENTRIES: [hal::binary_info::EntryAddr; 4] = [
    ...
    hal::binary_info::rp_program_description!(
        c"zkmcu: Plonky3 PQ-Semaphore dual-hash (Poseidon2 + Blake3) verify (Hazard3 RV32)"
    ),
...
fn main() -> ! {
    ...
    let mut bench = Bench::new(
        ...
        BenchConfig {
            manufacturer: "zkmcu",
            product: "bench-rp2350-rv32-pq-semaphore-dual",
            serial: "0001",
        },
    );
    ...
    let _ = writeln!(
        &mut boot_line,
        "zkmcu boot: heap=384K sys=150MHz core=hazard3 alloc=tlsf proof={VEC_LABEL}\r",
    );
