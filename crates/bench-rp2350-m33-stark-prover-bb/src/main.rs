//! Phase-5: BabyBear + Quartic-extension STARK Fibonacci prover on Cortex-M33.
//!
//! Companion to `bench-rp2350-m33-stark-prover` (Phase 4, Goldilocks + None).
//! The only differences are the base field (31-bit BabyBear vs 64-bit Goldilocks)
//! and the field extension (Quartic vs None). The circuit, trace, and FRI
//! domain parameters are identical so results are directly comparable.
//!
//! BabyBear elements are 4 bytes each (vs 8 for Goldilocks), so the LDE
//! matrix is half the size. The composition polynomial uses the quartic
//! extension (16 bytes per element), so that portion is 2× Goldilocks.
//! Net heap footprint is measured here for the first time.
//!
//! BabyBear multiplication is a single UMULL on M33 and MUL+MULHU on RV32
//! — both ISAs handle 32-bit equally. The cross-ISA prove gap seen in
//! Phase 4 (1.55×, caused by 64-bit UMAAL vs paired mul/mulhu for
//! Goldilocks) should collapse here.
...
pub static PICOTOOL_ENTRIES: [hal::binary_info::EntryAddr; 4] = [
    ...
    hal::binary_info::rp_program_description!(
        c"zkmcu: STARK Fibonacci PROVER BabyBear+Quartic (Cortex-M33)"
    ),
...
// N=512 at blowup=4 needs ~496 KB heap — exceeds the 512 KB SRAM ceiling.
...
fn main() -> ! {
    ...
    let mut bench = Bench::new(
        ...
        BenchConfig {
            manufacturer: "zkmcu",
            product: "bench-rp2350-m33-stark-prover-bb",
            serial: "0001",
        },
    );
    ...
    let _ = writeln!(
        &mut boot_line,
        "zkmcu boot: heap=384K sys=150MHz core=cortex-m33 alloc=tlsf stark-prover-bb N={TRACE_LEN}\r",
    );
