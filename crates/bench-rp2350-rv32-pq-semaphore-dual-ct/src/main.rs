...
// 384 KB heap. Same budget as the M33 sibling; CT path preserves the
// drop-between pattern from `pq_semaphore_dual` so peak heap stays at
// `max(p2_peak, b3_peak)` ≈ 304 KB. Leaves ~120 KB stack region for
// `measure_stack_peak`'s 64 KB sentinel paint.
...
pub static PICOTOOL_ENTRIES: [hal::binary_info::EntryAddr; 4] = [
    ...
    hal::binary_info::rp_program_description!(
        c"zkmcu: Plonky3 PQ-Semaphore dual-hash CT verify (Hazard3 RV32)"
    ),
...
fn main() -> ! {
    ...
    let mut bench = Bench::new(
        ...
        BenchConfig {
            manufacturer: "zkmcu",
            product: "bench-rp2350-rv32-pq-semaphore-dual-ct",
            serial: "0001",
        },
    );
    ...
    let _ = writeln!(
        &mut boot_line,
        "zkmcu boot: heap=384K sys=150MHz core=hazard3 alloc=tlsf proof={VEC_LABEL} mode=ct\r",
    );
