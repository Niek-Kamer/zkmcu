...
pub static PICOTOOL_ENTRIES: [hal::binary_info::EntryAddr; 4] = [
    ...
    hal::binary_info::rp_program_description!(
        c"zkmcu: Plonky3 PQ-Semaphore Goldilocks-Quadratic verify (Cortex-M33)"
    ),
...
fn main() -> ! {
    ...
    let mut bench = Bench::new(
        ...
        BenchConfig {
            manufacturer: "zkmcu",
            product: "bench-rp2350-m33-pq-semaphore-gl",
            serial: "0001",
        },
    );
    ...
    let _ = writeln!(
        &mut boot_line,
        "zkmcu boot: heap=480K sys=150MHz core=cortex-m33 alloc=tlsf proof={VEC_LABEL}\r",
    );
    ...
    // Skip stack-paint boot measurement: the 480 KB heap leaves only
    // ~32 KB of stack region above .bss, but `measure_stack_peak`
    // paints a 64 KB sentinel below SP. The paint overflows into the
    // heap and corrupts allocator state, panicking on the first verify
    // allocation. Heap peak is captured below from the regular loop
    // via `HEAP.peak()` instead. Same trade-off the Phase C reject
    // benches accepted (stack peak not captured for harness simplicity).
